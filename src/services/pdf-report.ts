...
function documentHtml(title: string, subtitle: string, body: string): string {
  ...
  <style>
    ...
    .metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:12px 0}.metrics div{padding:8px 10px;border-radius:9px;background:#f1f5f9}.metrics span{display:block;color:#64748b;font-size:10px}.metrics strong{display:block;margin-top:2px}.report-cover{break-after:page;page-break-after:always}.report-cover h2{margin:22px 0 4px;font-size:17px}.report-cover p{max-width:470px;margin:0;color:#64748b}.report-records{display:block}
...
export function buildAllPdfReport(
  ...
  const cover = `<section class="report-cover">
    ${summary}
    <h2>Report overview</h2>
    <p>The detailed Personal and Work records begin on the next page for consistent, readable pagination.</p>
  </section>`;
  return {
    ...
    html: documentHtml(
      ...
      `${cover}<div class="report-records">${sections}</div>`,
    ),
  };
}
