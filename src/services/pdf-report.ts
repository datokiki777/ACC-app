...
function documentHtml(title: string, subtitle: string, body: string): string {
  ...
  <style>
    ...
    .metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:12px 0}.metrics div{padding:8px 10px;border-radius:9px;background:#f1f5f9}.metrics span{display:block;color:#64748b;font-size:10px}.metrics strong{display:block;margin-top:2px}.report-records{display:block;margin-top:16px}.report-group{display:block}.report-group+.report-group{margin-top:18px}.report-group-title{break-after:avoid;page-break-after:avoid;margin:0 0 9px;padding:0 2px 6px;border-bottom:2px solid #ffc107;color:#0b1c32;font-size:16px}
...
export function buildAllPdfReport(
  ...
  const sections = [
    data.personal.length
      ? `<section class="report-group"><h2 class="report-group-title">Personal records</h2>${data.personal
          .map((person) => personSection(person, 'personal', referenceDate))
          .join('')}</section>`
      : '',
    data.work.length
      ? `<section class="report-group"><h2 class="report-group-title">Work teams</h2>${data.work
          .map((person) => personSection(person, 'work', referenceDate))
          .join('')}</section>`
      : '',
  ].join('');
  return {
    ...
    html: documentHtml(
      ...
      `${summary}<div class="report-records">${sections}</div>`,
    ),
  };
}
